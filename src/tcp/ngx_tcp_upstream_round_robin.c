...
static ngx_int_t ngx_tcp_upstream_cmp_servers(const void *one,
    const void *two);
static ngx_tcp_upstream_rr_peer_t *ngx_tcp_upstream_get_peer(
    ngx_tcp_upstream_rr_peer_data_t *rrp);

#if (NGX_TCP_SSL)

static ngx_int_t ngx_tcp_upstream_empty_set_session(ngx_peer_connection_t *pc,
    void *data);
static void ngx_tcp_upstream_empty_save_session(ngx_peer_connection_t *pc,
    void *data);
    ...
    ngx_tcp_upstream_srv_conf_t *us)
{
    ...
    if (rrp == NULL) {
        rrp = ngx_palloc(r->pool, sizeof(ngx_tcp_upstream_rr_peer_data_t));
        if (rrp == NULL) {
            return NGX_ERROR;
        }
    ...
    r->upstream->peer.get = ngx_tcp_upstream_get_round_robin_peer;
    r->upstream->peer.free = ngx_tcp_upstream_free_round_robin_peer;
    ...
    ngx_tcp_upstream_resolved_t *ur)
{
    ...
    peers = ngx_pcalloc(r->pool, sizeof(ngx_tcp_upstream_rr_peers_t)
    ...
    r->upstream->peer.tries = rrp->peers->number;
#if (NGX_TCP_SSL)
    r->upstream->peer.set_session = ngx_tcp_upstream_empty_set_session;
    r->upstream->peer.save_session = ngx_tcp_upstream_empty_save_session;
#endif
...
ngx_tcp_upstream_get_round_robin_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (rrp->peers->single) {
        ...
        peer = ngx_tcp_upstream_get_peer(rrp);
...
static ngx_tcp_upstream_rr_peer_t *
ngx_tcp_upstream_get_peer(ngx_tcp_upstream_rr_peer_data_t *rrp)
{
    time_t                        now;
    uintptr_t                     m;
    ngx_int_t                     total;
    ngx_uint_t                    i, n;
    ngx_tcp_upstream_rr_peer_t  *peer, *best;
...
static ngx_int_t
ngx_tcp_upstream_empty_set_session(ngx_peer_connection_t *pc, void *data)
{
    return NGX_OK;
}
...
static void
ngx_tcp_upstream_empty_save_session(ngx_peer_connection_t *pc, void *data)
{
    return;
}
